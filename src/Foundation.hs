...
data App = App
    ...
    , appStatic      :: Static
    , appConnPool    :: SQL.ConnectionPool
    , appHttpManager :: Manager
    , appLogger      :: Logger
    , appPractice    :: STM.Map (Key User) Game
...
instance Yesod App where
    ...
    shouldLogIO :: App -> Logger.LogSource -> LogLevel -> IO Bool
    shouldLogIO _ _ LevelWarn  = return True
    shouldLogIO _ _ LevelError = return True
    shouldLogIO app _source _  = return $ appShouldLogAll (appSettings app)
...
instance YesodPersist App where
    ...
    runDB action = do
        master <- getYesod
        SQL.runSqlPool action $ appConnPool master
    ...
    sendVerifyEmail email _ verurl =
      ...
      where
        vtextPart = Part
            ...
            , partContent = Encoding.encodeUtf8 [stext|
