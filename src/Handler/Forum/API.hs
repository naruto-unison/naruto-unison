...
getDeletePostR postId = attemptMaybeT do
    ...
    lift . runDB $ update postId [ForumPostDeleted =. True]
...
getLike :: Key ForumPost -> Key User -> SqlPersistT Handler (Maybe (Entity ForumLike))
getLike post who =
...
getDeleteTopicR topicId = attemptMaybeT do
    ...
    guard $ userPrivilege user > Normal
    lift . runDB $ update topicId [ForumTopicState =. Deleted]
...
setTopicState state topicId = attemptMaybeT do
    ...
    lift . runDB $ update topicId [ForumTopicState =. state]
    return ()
