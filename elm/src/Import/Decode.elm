...
customDecoder decoder toResult =
    decoder
    |> D.andThen (\a -> case toResult a of
                            Ok b    -> D.succeed b
                            Err err -> D.fail <| D.errorToString err)

decodeSumFinal : String -> String -> D.Value -> Dict String (D.Decoder a)
              -> Result D.Error a
decodeSumFinal name key val mapping =
    case Dict.get key mapping of
        Nothing  -> Err <| D.Failure
                    ("Unknown constructor " ++ key ++ " for type " ++ name) val
        Just dec -> D.decodeValue dec val
